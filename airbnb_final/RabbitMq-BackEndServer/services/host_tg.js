...
exports.allsteps_ng= function (msg,callback) {
	...
	if(msg.guest_place=="Shared Room"){
		...
		console.log("in googlemaps");
		  ...
		  var query1= "INSERT INTO bid (property_id, UserFname, UserLname, HostFname, HostLname, bidPrice, bidTime, bid, User_email, user_id, host_id,guests,checkin,checkout,type,city,state,apt,zip,Street, guest_place, place, total, country) VALUES ( '"+propertyid+"','Nayan','Goel','Nayan','Goel','"+price+"','"+bid_endtime+"','"+msg.bid+"','nayangoel@gmail.com','007-08-1234','"+msg.hostid+"','"+msg.guestnum+"','9999-99-99','9999-99-99','"+msg.type+"','"+msg.city+"','"+msg.state+"','"+msg.apt+"','"+msg.zip+"','"+msg.street+"','"+msg.guest_place+"','"+msg.place+"','"+total_bid+"','"+msg.country+"');";
	mysql.pushData(function (err,result1) {
		if(err){
			throw err;
		}
		else{
				var query2= "INSERT INTO bookedprop(property_id, UserName, Qty, checkin, checkout) VALUES ('"+propertyid+"','Nayan','"+qty+"','9999-99-99','9999-99-99');";
				...
				if(err){
					...
					mysql.fetchData(function (err, tripdata) {
						if(err){
							...
							var query3= "INSERT INTO bookedprop(property_id, UserName, Qty, checkin, checkout,Trip_id) VALUES ('"+result1[0].property_id+"','"+result1[0].UserName+"','"+result1[0].Qty+"','"+result1[0].checkin+"','"+result1[0].checkout+"','"+gottrip_id+"');";
							mysql.pushData(function (err,result2) {
								if(err){
									throw err;
								}
...
exports.gethostreviewsbyyou_ng = function(msg, callback){
 ...
 //                                        var coll_propertyReview = mongo.collection('propertyReview');
 //                                        coll_propertyReview.find({host_id: msg.hostid}).toArray(function (err, property) {
 //                                            if (err) {
 //                                                throw err;
 //                                            } else {
 ...
 //                                                var coll_propertyReview = mongo.collection('propertyReview');
 //                                                coll_propertyReview.count({host_id: msg.hostid}, function (err, cntt) {
 //                                                    if (err) {
 //                                                        throw err;
 //                                                    } else {
